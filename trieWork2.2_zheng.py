...
for idx,alias in enumerate(aliasset):
    ...
    if len(firfam) > 3:
        temp = []
        temp.append(firfam[0]+" "+firfam[1]+" "+firfam[2]) 
        strtemp = firfam[3]
                ...
                for j in range(len(famName[0])):
                    ...
                    if filename in famName[0][j]:
                        ...
                        if fam_start_num - fir_end_num == 2:
                            with open("formatLink2.1_z2.txt", 'a',encoding='utf-8') as file:
        ...
        if(B.exists(alias)): 
            ...
            for i in range(len(firfam)):
                if(D.exists(firfam[i].lower()) or C.exists(firfam[i].lower())):
                    allWordsUnNoun -= 1
            if allWordsUnNoun == 0:
                continue 
            else:
        ...
        if(B.exists(alias) and not D.exists(alias.lower()) and not alias.isdigit()):
